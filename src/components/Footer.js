                ...
                <div className="w-full md:w-6/12 mb-8">
                  ...
                  <ul className="list-reset">
                    { headerNavigationItems.map(item =>
                      <li className="mb-3">
                        <Link key={item.wordpress_id} className="text-white text-lg font-semibold no-underline" activeClassName="text-blue-lightest" to={slugify(item.title, {lower: true})}>{item.title}</Link>
                      </li>
                      )
                    }
